...
BENCHMARK_NAMES = wiki-fair-no-coref wiki-fair wiki-fair-v2 wiki-fair-v2-no-coref
...
link-wiki:
	...
	@if ls ${LINKED_WIKI_ARTICLES} 1> /dev/null 2>&1; then echo -e "$${RED}Linked Wikipedia dump already exists at ${LINKED_WIKI_ARTICLES} . Delete or rename it if you want to link another dump. Dump not linked.$${RESET}"; echo; else \
	  python3 link_text.py ${EXTRACTED_WIKI_DUMP} ${LINKED_WIKI_ARTICLES} -l popular-entities -hl hyperlink-reference -coref kb-coref -m ${NUM_LINKER_PROCESSES}; \
	fi
